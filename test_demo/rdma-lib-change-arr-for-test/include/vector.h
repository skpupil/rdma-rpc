...
public:
  ...
  { try_init(); conn = connection; ele = data_allocator::allocate(1); num_entry = num_entrys; }

  explicit vector(client_connection* connection, size_type n, uint64_t num_entrys)
  { fill_init(n, value_type()); conn = connection; num_entry = num_entrys;}

  vector(client_connection* connection, size_type n, const value_type& value, uint64_t num_entrys)
  { fill_init(n, value); conn = connection; num_entry = num_entrys;}
...
public:
  ...
  {const value_type val = value; push_back(val);}
...
void vector<T>::emplace_back(Args&& ...args)
{
  ...
  {
    //data_allocator::construct(mystl::address_of(*end_), mystl::forward<Args>(args)...);
    int index = end_ - begin_;
    if(index < num_entry){
    	data_allocator::construct(mystl::address_of(*end_), mystl::forward<Args>(args)...);
	bit_map.set(index, 1);
	debug("emplace back at local\n");
    }else{
	//TODO: klxchange here
	//https://www.cnblogs.com/qicosmos/p/4325949.html
	//value_type val = va_arg(args, value_type);//args[0];
        //conn->rdma_write(&val, st_addr + (end_ - begin_) * sizeof(T), sizeof(T));
	bit_map.set(index, 0); 
	debug("emplace back at remote is unimplemented\n\n\n\n\n");
    }
    ++end_;
  }
  else
...
void vector<T>::init_space(size_type size, size_type cap)
{
  try
  {
    ...
    Slab& slab = Slab::get_instance(); 
    st_addr = slab.get_mem(cap * sizeof( T ));
    end_ = begin_ + size;
    cap_ = begin_ + cap;
  }
  catch (...)
