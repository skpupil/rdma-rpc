...
class vector_local
{
  static_assert(!std::is_same<bool, T>::value, "vector_local<bool> is abandoned in mystl");
public:
  // vector_local 的嵌套型别定义
...
public:
  // 构造、复制、移动、析构函数
  vector_local() noexcept
  { try_init(); }

  explicit vector_local(size_type n)
  { fill_init(n, value_type()); }

  vector_local(size_type n, const value_type& value)
  { fill_init(n, value); }

  template <class Iter, typename std::enable_if<
    mystl::is_input_iterator<Iter>::value, int>::type = 0>
  vector_local(Iter first, Iter last)
  {
    MYSTL_DEBUG(!(last < first));
    range_init(first, last);
  }

  vector_local(const vector_local& rhs)
  {
    range_init(rhs.begin_, rhs.end_);
  }

  vector_local(vector_local&& rhs) noexcept
    :begin_(rhs.begin_),
    end_(rhs.end_),
    cap_(rhs.cap_)
  {
  ...
  vector_local(std::initializer_list<value_type> ilist)
  {
    range_init(ilist.begin(), ilist.end());
  }

  vector_local& operator=(const vector_local& rhs);
  vector_local& operator=(vector_local&& rhs) noexcept;

  vector_local& operator=(std::initializer_list<value_type> ilist)
  {
    vector_local tmp(ilist.begin(), ilist.end());
    swap(tmp);
    return *this;
  }
...
public:
  ...
  {
    THROW_OUT_OF_RANGE_IF(!(n < size()), "vector_local<T>::at() subscript out of range");
    return (*this)[n];
  }
  ...
  // swap
  void     swap(vector_local& rhs) noexcept;
...
// 复制赋值操作符
template <class T>
vector_local<T>& vector_local<T>::operator=(const vector_local& rhs)
{
  if (this != &rhs)
  {
    ...
    if (len > capacity())
    { 
      vector_local tmp(rhs.begin(), rhs.end());
      swap(tmp);
    }
    else if (size() >= len)
    {
...
// 移动赋值操作符
template <class T>
vector_local<T>& vector_local<T>::operator=(vector_local&& rhs) noexcept
{
  destroy_and_recover(begin_, end_, cap_ - begin_);
  begin_ = rhs.begin_;
  end_ = rhs.end_;
...
template <class T>
void vector_local<T>::reserve(size_type n)
{
  if (capacity() < n)
  {
    THROW_LENGTH_ERROR_IF(n > max_size(),
                          "n can not larger than max_size() in vector_local<T>::reserve(n)");
...
// 放弃多余的容量
template <class T>
void vector_local<T>::shrink_to_fit()
{
  if (end_ < cap_)
  {
    reinsert(size());
...
template <class T>
template <class ...Args>
typename vector_local<T>::iterator
vector_local<T>::emplace(const_iterator pos, Args&& ...args)
{
  MYSTL_DEBUG(pos >= begin() && pos <= end());
...
template <class T>
template <class ...Args>
void vector_local<T>::emplace_back(Args&& ...args)
{
  if (end_ < cap_)
  {
    data_allocator::construct(mystl::address_of(*end_), mystl::forward<Args>(args)...);
...
// 在尾部插入元素
template <class T>
void vector_local<T>::push_back(const value_type& value)
{
  if (end_ != cap_)
  {
...
template <class T>
typename vector_local<T>::iterator
vector_local<T>::insert(const_iterator pos, const value_type& value)
{
  MYSTL_DEBUG(pos >= begin() && pos <= end());
  iterator xpos = const_cast<iterator>(pos);
...
template <class T>
typename vector_local<T>::iterator
vector_local<T>::erase(const_iterator pos)
{
  MYSTL_DEBUG(pos >= begin() && pos < end());
...
template <class T>
typename vector_local<T>::iterator
vector_local<T>::erase(const_iterator first, const_iterator last)
{
...
// 重置容器大小
template <class T>
void vector_local<T>::resize(size_type new_size, const value_type& value)
{
  if (new_size < size())
  {
...
// 与另一个 vector_local 交换
template <class T>
void vector_local<T>::swap(vector_local<T>& rhs) noexcept
{
  if (this != &rhs)
  {
...
template <class T>
void vector_local<T>::try_init() noexcept
{
  try
  {
...
// init_space 函数
template <class T>
void vector_local<T>::init_space(size_type size, size_type cap)
{
  try
  {
...
// 重新分配空间并在 pos 处插入元素
template <class T>
void vector_local<T>::reallocate_insert(iterator pos, const value_type& value)
{
  const auto new_size = get_new_cap(1);
  auto new_begin = data_allocator::allocate(new_size);
  auto new_end = new_begin;
...
// reinsert 函数
template <class T>
void vector_local<T>::reinsert(size_type size)
{
...
template <class T>
bool operator==(const vector_local<T>& lhs, const vector_local<T>& rhs)
{
...
// 重载 mystl 的 swap
template <class T>
void swap(vector_local<T>& lhs, vector_local<T>& rhs)
{
  lhs.swap(rhs);
}
